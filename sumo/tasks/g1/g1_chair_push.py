...
XML_PATH = str(MODEL_PATH / "xml/g1/g1_chair.xml")

Z_AXIS = np.array([0.0, 0.0, 1.0])
CHAIR_HEIGHT = 0.0  # Height of the chair base from the ground
...
# Arm usage configuration for chair manipulation
USE_LEFT_ARM = False
USE_RIGHT_ARM = False
...
@dataclass
class G1ChairPushConfig(G1BaseConfig):
    """Config for the G1 chair push task."""

    goal_position: np.ndarray = np_1d_field(
        np.array([0.0, 0.0, CHAIR_HEIGHT], dtype=np.float64),
        names=["x", "y", "z"],
        mins=[-5.0, -5.0, 0.0],
        maxs=[5.0, 5.0, 1.0],
...
class G1ChairPush(G1Base):
    config_t = G1ChairPushConfig
    config: G1ChairPushConfig

    """Task getting G1 to push a yellow chair to a desired goal location.
    ...
    def __init__(self, model_path: str = XML_PATH) -> None:
        ...
        self.object_z_axis_idx = get_sensor_indices(self.model, "chair_z_axis")
    ...
    def reward(
        ...
        """Reward function for the G1 chair pushing task."""
        ...
        # Compute l2 distance from object pos. to goal (XY only for chair pushing)
        goal_reward = -config.w_goal * np.linalg.norm(
            object_pos[..., :2] - np.array(config.goal_position[:2])[None, None], axis=-1
        ).mean(-1)

        # Chair orientation reward - penalize if chair is not upright
        chair_orientation_reward = -config.w_orientation * np.abs(np.dot(object_z_axis, Z_AXIS) - 1.0).sum(axis=-1)
        ...
        # Compute l2 distance from hands to object - encourage getting hands close to chair
        ...
        assert chair_orientation_reward.shape == (batch_size,)
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        # Sample chair position in front of robot
        object_pos = np.array([-1, 0])  # Chair positioned in front of robot

        # Chair orientation (upright by default)
        object_orientation = np.array([1, 0, 0, 0])

        reset_object_pose = np.array([*object_pos, CHAIR_HEIGHT, *object_orientation])
